...
function optionSelected(answer) {
    ...
    let category = questions[que_count].category; // Getting category of current question

    if (userAns == correcAns) { // If user selected option is equal to array's correct answer
        userScore += 1; // Upgrading score value with 1

        // Update category-specific scores
        if (category === "Algebra") algebraScore += 1;
        if (category === "Analysis") analysisScore += 1;
        if (category === "Optimization") optimizationScore += 1;
        if (category === "ProbaStat") probaStatScore += 1;

        answer.classList.add("correct"); // Adding green color to correct selected option
        answer.insertAdjacentHTML("beforeend", tickIconTag); // Adding tick icon to correct selected option
        console.log("Correct Answer");
        console.log("Your correct answers = " + userScore);
        console.log(`Scores: Algebra - ${algebraScore}, Analysis - ${analysisScore}, Optimization - ${optimizationScore}, ProbaStat - ${probaStatScore}`);
    } else {
